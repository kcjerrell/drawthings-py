...
ENUM_SCHEMA = MapPattern(Str(), Map({
    "type": Enum(["str"]),
    "fbs_type": Enum(["int"]),
    "values": MapPattern(Str(), Int()),
...
def gen_enums():
    yaml_path = files("drawthings_py.resources").joinpath("enums.yaml")
    yaml_text = yaml_path.read_text()
    enums_yaml = load(yaml_text, ENUM_SCHEMA).data

    enums = []

    for name, enum in enums_yaml.items():
        values = list(enum["values"].keys())

        enums.append({
            "name": name,
            "snake_name": _snake_case(name),
            "enum_values": values,
            "proper_list": f"{_snake_case(name).upper()}_VALUES",
            "lookup_table": f"_{_snake_case(name).upper()}_LOOKUP",
            "default_name": f"_{_snake_case(name).upper()}_DEFAULT",
            "default_value": enum.get("default", 0),
            "default_str": values[enum.get("default", 0)],
        })
    ...
    enums_template = env.get_template("enums.py.jinja")
    enums_header = dedent('''
        """
        ...
        """
        ''')
    enums_code = "\n".join([enums_header, enums_template.render(**context)])

    _ =Path("./src/drawthings_py/configs/enums.py").write_text(enums_code)

    _ = subprocess.run(
        ["ruff", "format", "./src/drawthings_py/configs/enums.py"],
        check=True,
    )
