...
def _validate_pattern(pattern: str) -> None:
    ...
    if len(placeholders) > 1:
        raise ValueError("Pattern must include exactly one # block for the item counter")
    if "/" in pattern:
        raise ValueError("Pattern should represent filename only, and cannot contain '/'")
